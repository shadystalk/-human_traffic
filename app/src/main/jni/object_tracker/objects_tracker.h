...
public:
    ...
    struct ExtObject {
        ...
        ExtObject(int _id, Rect_T _location, Rect_T _predict_loc_when_miss, Rect_T _smooth_rect,
                  ObjectStatus _status,
                  int _detectedNum, int _noDetectedNum, int _obj_class, float _score, int _miss)
                : id(_id), location(_location), predict_loc_when_miss(_predict_loc_when_miss),
                  smooth_rect(_smooth_rect), status(_status),
                  detectedNum(_detectedNum), noDetectedNum(_noDetectedNum), obj_class(_obj_class),
                  score(_score), miss(_miss) {
    ...
    void updateTrackedObjects(const std::vector<Rect_T> &detectedObjects,
                              const std::vector<int> objects_class,
                              const std::vector<float> objects_score, int maxTrackLifetime,
    ...
    struct TrackedObject {
        ...
        //int flag_unshift;*/
        TrackedObject(const Rect_T &Rect_T) : vx(0), vy(0), numDetectedFrames(1),
                                              numFramesDetected(1), numFramesNotDetected(0),
    ...
    void
    predict_loctation(TrackedObject &curObject, int image_width, int image_height, float &pre_x,
                      float &pre_y, float &v_x, float &v_y);
