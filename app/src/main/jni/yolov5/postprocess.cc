...
static float
CalculateOverlap(float xmin0, float ymin0, float xmax0, float ymax0, float xmin1, float ymin1,
...
static int
process(int8_t *input, int *anchor, int grid_h, int grid_w, int height, int width, int stride,
...
static int
process_i8(int8_t *input, int *anchor, int grid_h, int grid_w, int height,
...
static int
process_fp(float *input, int *anchor, int grid_h, int grid_w, int height,
           int width, int stride,
           std::vector<float> &boxes, std::vector<float> &boxScores, std::vector<int> &classId,
           float threshold) {
    // printf("anchor :");
    ...
    for (int a = 0; a < 3; a++) {
        for (int i = 0; i < grid_h; i++) {
            for (int j = 0; j < grid_w; j++) {
                float box_confidence = input[(YOLOV5_PROP_BOX_SIZE * a + 4) * grid_len +
                                             i * grid_w + j];
...
//    LOGI("qnt_scales: [%f, %f, %f]", qnt_scales[0], qnt_scales[1], qnt_scales[2]);
    ...
    validCount0 = process_i8(input0, (int *) anchor0, grid_h0, grid_w0, model_in_h, model_in_w,
                             stride0, filterBoxes, objProbs, classId, conf_threshold, qnt_zps[0],
                             qnt_scales[0]);
    ...
    validCount1 = process_i8(input1, (int *) anchor1, grid_h1, grid_w1, model_in_h, model_in_w,
                             stride1, filterBoxes, objProbs, classId, conf_threshold, qnt_zps[1],
                             qnt_scales[1]);
    ...
    validCount2 = process_i8(input2, (int *) anchor2, grid_h2, grid_w2, model_in_h, model_in_w,
                             stride2, filterBoxes, objProbs, classId, conf_threshold, qnt_zps[2],
                             qnt_scales[2]);
...
//        LOGI("post process end.");
    ...
    for (int i = 0; i < validCount; ++i) {
        ...
        if (id == 0) {
           ...
           // char *label = yolov5_lables[id];
           // strncpy(group->results[last_count].name, label, YOLOV5_OBJ_NAME_MAX_SIZE);
            LOGI("result %2d: (%4d, %4d, %4d, %4d), %f\n", id, group->results[last_count].box.left,
                 group->results[last_count].box.top,
                 group->results[last_count].box.right, group->results[last_count].box.bottom,
                 group->results[last_count].prop);
            last_count++;
//        }
        }
    }
